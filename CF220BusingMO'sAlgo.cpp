...
int sz,res[SIZE],ar[SIZE],couunt=0;
...
void add(int index){
	...
	else if(cnt[v]>v and cnt[v]-1==v) couunt--;
	//cout<<"FI: "<<v<<" "<<cnt[v]<<" "<<couunt<<endl;
}
void minu(int index){
	LL v=ar[index];
	cnt[v]--;
	...
	else if(cnt[v]<v and cnt[v]+1==v and check[v]){
		couunt--;
	}
	//cout<<"SEC: "<<v<<" "<<cnt[v]<<" "<<couunt<<endl;
}
